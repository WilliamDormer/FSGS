...
def render(viewpoint_camera, pc, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0,
           override_color = None, white_bg = False, debug=False, speed=False):
    """
    Render the scene.
    ...
    if not speed:
        ...
        p_view = p_view[...,:3,:]
        # My change: New Depth
        depth_new_shader = p_view.squeeze()[...,2:3] 
        depth_new_shader = depth_new_shader.repeat(1,3)

        render_extras = {"depth_new_shader": depth_new_shader}
        if debug: 
